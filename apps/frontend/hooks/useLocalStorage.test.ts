...
describe('useLocalStorage', () => {
  ...
  describe('setGameSession', () => {
    test('should store gameCode and playerId', () => {
      const { result } = renderHook(() => useLocalStorage());

      act(() => {
        result.current.setGameSession('WXYZ', 'player-456');
      });
    ...
    test('should overwrite existing values', () => {
      ...
      act(() => {
        result.current.setGameSession('NEW', 'new-player');
      });
  ...
  describe('clearSession', () => {
    test('should remove all session data from localStorage, including the legacy player name', () => {
      window.localStorage.setItem('gameCode', 'ABCD');
      window.localStorage.setItem('playerId', 'player-123');
